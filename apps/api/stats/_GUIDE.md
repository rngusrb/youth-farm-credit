...
- **KAMIS 관측일수로 계절성을 재지 않는다.** 거래가 없어도 직전 시세가 이월되므로
  관측일수는 달의 영업일수만 반영한다. 사고 이력: 21작목 중 17개가 최저/최대 비율
  0.86 으로 동일했는데, 그 0.86 은 2월(20영업일)과 3월(23영업일)의 차이였다.
- **이월 시세가 섞인 품목에 국면 판정을 붙이지 않는다.** `price_movement_ratio`
  60% 미만이면 `regime` 이 `None` 이다. 근거는 `docs/decisions/0002`.

## 하네스

```
apps/api/tests/test_stats.py
apps/api/tests/test_factors.py
apps/api/tests/test_kosis.py
...
## 모듈 지도 — adapters — 통계·수집

| 모듈 | 역할 |
|---|---|
| `env.py` | .env 로더. 인증키는 환경변수로만 다룬다 |
| `factors.py` | 소득 변동의 가격/수량/비용 요인분해. KOSIS 행 타입에 의존한다 |
| `kosis.py` | KOSIS 농산물소득조사 클라이언트. 표마다 축 순서가 다르다 |
