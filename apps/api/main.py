...
# httpx 는 요청 URL 을 통째로 INFO 로그에 남긴다. 공공데이터포털 API 는 인증키를
# **쿼리스트링**으로 받으므로, 그대로 두면 서버 로그에 키가 평문으로 쌓인다.
#   INFO:httpx:HTTP Request: GET https://apis.data.go.kr/...?serviceKey=pC115JF...
# 저장소가 공개라 .env 를 안 올렸는데 로그로 새면 의미가 없다. 특히 배포 후에는
# 로그가 남는다. (2026-09-06 서버 로그에서 실제로 노출된 것을 확인하고 막음)
logging.getLogger("httpx").setLevel(
    os.getenv("HTTPX_LOG_LEVEL", "WARNING").upper()
)

# 외부 시세 API 대기 상한. 화면이 여기에 매달린다 — 60초면 농가는 1분 동안
# 빈 화면을 본다. 자료가 안 와도 각 섹션은 "아직 모으고 있어요" 로 잘 내려가므로,
# 오래 기다리는 것보다 빨리 포기하는 편이 낫다.
# 동료도 같은 방향으로 두 번 고쳤다 (a8de30e "fail fast on delayed recent api",
# f040b10 "keep auction summary responsive when api is slow") — monthly·quarterly 에만
...
def latest_priced_date(records: list[dict], date_key: str = "가격날짜",
                       price_key: str = "평균가격") -> str:
    """가격이 실제로 있는 **최신 날짜**. 없으면 그냥 최신 날짜.

    사고 이력 2026-09-06: "최신 날짜" 와 "가격이 있는 최신 날짜" 를 같은 것으로 봤다.
    딸기 2024-08-29 는 8건이 전부 평균가격 0 이었고(비수기, 출하 없음),
    화면에 날짜만 있고 가격은 빈칸인 "최근 가격" 이 떴다.

    순수 함수로 빼 둔 이유: 이 판단을 검사하려고 외부 API 를 때리면 32초가 걸리고
    자료가 안 오는 날엔 검사가 조용히 skip 된다. 판단은 여기서, 통신은 저기서.
    """
    def has_price(row: dict) -> bool:
        try:
            return float(str(row.get(price_key, "")).replace(",", "")) > 0
        except (TypeError, ValueError):
            return False

    priced = [str(r.get(date_key, "")) for r in records if has_price(r)]
    if priced:
        return max(priced)
    return max((str(r.get(date_key, "")) for r in records), default="")
...
async def market_compare(crop_id: str | None = Query(default=None)) -> dict:
    ...
    # 페이지를 **끝까지** 읽는다. 1페이지만 받으면 오래된 1000건만 손에 쥐고
    # 그 안에서 max() 를 돌리게 된다 — 딸기 기준 실제 최신은 2024-08-29 인데
    # 화면에 2024-06-11 이 "최근 가격" 으로 떴다 (2026-09-06 확인).
    # totalCount 는 품목 필터 **이전** 전체 건수라 종료 조건으로 쓸 수 없다.
    # 빈 페이지가 나올 때까지 읽되, 폭주하지 않게 상한을 둔다.
    MAX_PAGES = 20
    records: list[dict] = []
    try:
        async with httpx.AsyncClient(timeout=MARKET_TIMEOUT_S) as client:
            for page_no in range(1, MAX_PAGES + 1):
                response = await client.get(endpoint, params={
                    "serviceKey": unquote(key), "page": page_no, "perPage": 1000,
                ...
                log.warning("공판장 가격 %s: %d 페이지 상한에 걸렸다 — 더 최신 자료가 있을 수 있다",
                            query_name, MAX_PAGES)
    except (httpx.HTTPError, ValueError):
        return {"status": "unavailable", "items": []}
    needles = [a.replace("(시설,수경)", "").replace("(시설,토경)", "") for a in aliases]
    ...
    # **가격이 있는** 최신일을 고른다. 그냥 최신일을 잡으면 비수기가 걸린다 —
    # 딸기 2024-08-29 는 8건이 다 평균가격 0 이었고(출하 없음), 화면에 날짜만 있고
    # 가격은 빈칸인 "최근 가격" 이 떴다 (2026-09-06 확인).
    latest_date = latest_priced_date(matched)
