...
**선언한 좌표가 실제 출력에 존재하는지**를 기계가 검사한다. 이게 없으면 엔진 출력
구조가 바뀌었을 때 아무도 모른다 (실제로 `income.expected` 오타를 이 검사가 잡았다).

⚠️ 정정 (2026-09-02, 적대적 리뷰 H2): 여기 원래 "Verifier 가 이 좌표로 문장 속 숫자를
대조한다" 고 적혀 있었는데 **사실이 아니다.** `llm/verify.py` 는 `returns` 를 읽지 않고,
결과 dict 의 모든 수치 리프를 모아 대조한다. 그러니 이 파일이 지키는 것은
"선언과 구현이 어긋나지 않는다" 이지 "검증이 좌표 기반이다" 가 아니다.
실제 검증 강도는 `engine/tools.py` 헤더에 실측과 함께 적어 두었다.
"""
import pytest
