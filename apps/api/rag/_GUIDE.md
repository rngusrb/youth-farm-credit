...
### ❌ 확장어를 상한 없이 붙이기
```python
# ❌ 금지 — LLM 이 낸 것을 그대로 12개
return tuple(dict.fromkeys(words))[:12]

# ✅ 대신 — 용어집 경로와 같은 상한 + 코퍼스 존재 검사
return tuple(_most_discriminative(words, MAX_ADDED))
```
**사고 이력**: 2026-09-01 키를 켜자 recall@1 이 6→4 로 떨어졌다. `MAX_ADDED=2` 상한이
용어집 경로에만 걸려 있고 LLM 경로엔 배선되지 않아 10개가 붙었다. 코드 주석이
"7개를 붙이면 정답이 10위 밖으로 밀린다"고 이미 경고하고 있었다.

### ❌ 코퍼스에 없는 확장어 쓰기
**사고 이력**: 같은 날. `_most_discriminative` 는 문서빈도 오름차순으로 고르는데,
코퍼스에 없는 조어('만세')는 df=0 이라 **가장 드문 말로 뽑혀 1순위 선택**된다.
노이즈가 최우선으로 채택되는 구조였다. df>0 필터를 앞에 뒀다.

### ❌ recall 회귀를 LLM 경로로 재기
**사고 이력**: 같은 날. LLM 질의확장은 같은 질문에 매번 다른 확장어를 낸다
(실측 3회: ('신청자격','연령') / ('지원자격','신청자격') / ('우선순위','신청자격')).
그 경로로 기준선을 재면 **회귀인지 운인지 구분할 수 없고** 매 실행마다 과금된다.
recall 테스트는 용어집 경로로 고정하고, LLM 출력 필터링은 가짜 응답으로 검사한다.
