          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <Panel>
              ...
              <DefTable rows={[
                ...
                // 라벨은 동료가 쉬운 말로 바꾼 것을 그대로 쓴다 (2026-09-06 병합).
                ["연 들어온 돈", <span key="a" className="tabular">{cf ? won(cf.annual.gross) : "—"}</span>,
                  scaled
                    ? { src: "assumed", note: `공표 10a당 들어온 돈 × 차주 신고 면적을, 차주 실적에 맞춰 ${cf!.income_basis.scale.toFixed(2)}배 조정했습니다. 비용 비율은 공표값 그대로입니다.` }
                    : { src: "public", note: "공표 10a당 들어온 돈 × 차주 신고 면적." }],
                ["농사 비용", <span key="b" className="tabular">{cf ? `− ${won(cf.annual.operating_cost)}` : "—"}</span>,
                  scaled
                    ? { src: "assumed", note: `같은 조사의 농사 비용을 들어온 돈과 같은 비율(${cf!.income_basis.scale.toFixed(2)}배)로 조정했습니다. 실제 비용 구조는 다를 수 있습니다.` }
