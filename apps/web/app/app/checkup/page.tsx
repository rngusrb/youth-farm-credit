...
export default function CheckupPage() {
  ...
  return (
    <>
      <PageTitle
        title="AI 농가 건강검진"
        lead="내가 번 돈을 같은 작물의 전국 평균과 비교해요. 농장 살림과 대출 갚을 준비도 함께 살펴보세요."
      />

      {error && <div className="mb-5"><Notice tone="danger">{error}</Notice></div>}

      {diag && (
        <Section title="계산에 사용한 내 소득">
          <IncomeSource d={diag} />
      ...
      <Section title="같은 작물의 전국 평균과 비교해요">
        {b?.comparable ? (
          <>
      ...
      {t && (
        <Section title="이 작물의 소득은 얼마나 달라질까요">
          <div className="grid gap-3 sm:grid-cols-3">
            <Stat
              label="농사 비용 비율"
            ...
            />
            <Stat label="소득이 달라지는 주된 이유" value={t.driver_label ?? "—"} note="가격·수확량·비용으로 나눠 분석" />
          </div>
          <p className="mt-2 text-[12px] leading-relaxed text-gov-ink3">
      ...
      {plan && diag && profile.plannedPyeong && (
        <Section title={`올해 계획대로 ${profile.plannedPyeong.toLocaleString()}평을 하면`}>
          <Panel>
            ...
            <p className="mt-2 text-[12px] leading-relaxed text-gov-ink3">
              면적만 바꿔 다시 계산한 값이에요. <b>시설 증설비와 늘어나는 농사 비용는 넣지
