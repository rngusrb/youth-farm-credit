...
export default function ReportsPage() {
  ...
  return (
    <>
      <PageTitle
        title="내 리포트"
        lead="진단 결과는 서버에 저장되지 않습니다. 문서번호(URL)에 입력값이 들어 있어 링크만 있으면 언제든 같은 리포트가 다시 계산됩니다. 이 목록은 이 브라우저의 기록입니다."
      />

      {rows.length === 0 ? (
        <Empty title="아직 저장된 리포트가 없습니다"
               body="진단을 실행하면 여기에 남습니다. 링크를 복사해 두면 다른 기기에서도 같은 리포트를 열 수 있습니다."
          ...
          <table className="w-full min-w-[680px] border-t border-gov-ink/70 text-[14px]">
            <thead>
              <tr className="bg-gov-sunk text-left text-[12px] font-semibold text-gov-ink2">
                ...
                <th scope="col" className="border-b border-gov-line px-4 py-3">감당 가능</th>
      ...
      <div className="mt-6">
        <Notice tone="info" title="기록은 이 브라우저에만 남습니다">
          최근 12건까지 보관합니다. 브라우저 데이터를 지우면 목록도 사라지지만, 리포트
          링크를 갖고 있으면 언제든 다시 열 수 있습니다.
        </Notice>
      </div>
    </>
  );
