...
const TONE = {
  danger: { bar: "bg-signal-danger", text: "text-signal-danger" },
  warn: { bar: "bg-signal-warn", text: "text-signal-warn" },
  ok: { bar: "bg-signal-ok", text: "text-signal-ok" },
} as const;
...
export default function LimitLadder({
  ...
  return (
    <section className="rounded-2xl border border-ink-700 bg-ink-900 p-6 sm:p-8">
        ...
        {rungs.map((r) => {
          ...
          return (
            <div key={r.key}>
              ...
              <div className="mt-1.5 h-2.5 w-full overflow-hidden rounded-full bg-ink-800">
                <div
                  className={`h-full rounded-full ${TONE[r.tone].bar}`}
                  style={{ width: `${share * 100}%` }}
                />
              </div>
              <div className="mt-1 flex flex-wrap justify-between gap-x-4 text-[11px] text-slate-600">
                <span>{r.basis}</span>
                {r.crisis !== null && (
      ...
      {livelihoodBound && (
        <p className="mt-6 border-t border-ink-800 pt-4 text-sm leading-relaxed text-slate-400">
          여기서 <span className="text-signal-danger">0원</span>은 계산 오류가 아닙니다.
          ...
          입니다. 지금 문제는 <strong className="text-slate-200">얼마를 빌리느냐</strong>가
          아니라 <strong className="text-slate-200">이 규모로 생계가 되느냐</strong>입니다.
      ...
      {!livelihoodBound && recommended > riskBased && (
        <p className="mt-6 border-t border-ink-800 pt-4 text-sm leading-relaxed text-slate-400">
          ...
          확률은{" "}
          <span className="tabular font-semibold text-signal-warn">
            {pct(crisisAtRecommended)}
          </span>
          입니다. DSCR은 소득을 매년 일정하다고 보고 계산하기 때문입니다. 소득이
          흔들리는 것까지 감안하면{" "}
          <span className="tabular font-semibold text-signal-ok">{won(riskBased)}</span>
          {" "}선입니다.
        </p>
      )}
    </section>
